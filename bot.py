    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                return {"error": f"Fathom API fout: {response.status_code}", "raw": response.text}
        except Exception as e:
            logger.error(f"Fathom request failed: {e}")
            return {"error": str(e)}
...
async def generate_todo_list(meetings_data):
    ...
    if isinstance(meetings_data, dict) and "error" in meetings_data:
        return f"⚠️ Kon calls niet ophalen: {meetings_data['error']}"
    meetings_text = str(meetings_data)
    message = anthropic_client.messages.create(
        model="claude-sonnet-4-20250514",
        max_tokens=1000,
        messages=[{"role": "user", "content": f"""Je bent een assistent die actiepunten filtert voor Finn Houwers uit meeting samenvattingen.
...
Geef alleen de to-do's voor Finn terug, geen andere informatie. Als er geen specifieke actiepunten voor Finn zijn, zeg dat dan."""}]
    )
...
async def send_daily_todo(bot: Bot):
    ...
    try:
        ...
        await bot.send_message(chat_id=CHAT_ID, text=f"⚠️ Er ging iets mis: {str(e)}")
...
async def todo_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_daily_todo(context.bot)
